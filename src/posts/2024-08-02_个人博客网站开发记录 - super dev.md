...
<span style="background:#eee">插播：后来我又找到一个包 [write-file-atomic](https://www.npmjs.com/package/write-file-atomic)，这是 npm 官方提供的（看来官方也觉得这很有必要，嗯哼），它能保证写入时的原子性。我用它测了一下，确实不会报错了。我不打算改方案，只是在这里提一下还有这个方法。</span>
