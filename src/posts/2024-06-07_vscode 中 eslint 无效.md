...
title: vscode 中 eslint 无效
createTime: 2024-06-07
updateTime:
categories: eslint
tags: vscode
description: 项目配置了 eslint，但是 vscode 并不能实时提示。研究后发现是 vscode 版本过旧，不能识别新版 eslint 的配置文件，升级 vscode 和其 eslint 插件即可解决。
---
...
果然没那么简单，哈哈哈。所以我去研究了一下 [npm init](post:6ebbba5f-9e24-4ead-a848-fe18bf8420c5) 的用法，知道了应该查看 `@eslint/create-config` 包（在上一部分安装@8 的报错信息也提到了这个包），看看这个包有没有哪个版本是对应 eslint8 的。

经过我一番查找，没有对应 eslint 8 的！~~这条路又堵死了，看来不能用自动化工具了，只能自己乖乖安包，自己写配置文件了~~（并不，升级 vscode 版本就好了）
